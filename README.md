...
| `NODE_ENV`              | `production` / `development` / `test`. Gates the secure-by-default anonymous block: with `AUTH_PROVIDER=none` and no `AUTH_ALLOW_ANONYMOUS=true`, anonymous access is refused only when this is `production`. Set automatically by the Docker image and by `next start` / `next dev` — set it manually only for non-standard launch paths. | `production` (Docker / `next start`) |
